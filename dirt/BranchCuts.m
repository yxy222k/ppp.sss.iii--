...
missed_residues=0;                                  %记录有效的残差点的个数？Keep track of the effective number ff residues left unbalanced because of
...
for i=1:temp(1)                                     %Loop through the residues
    ...
    if residue_balanced(r_active, c_active)~=1                        %Has this residue already been balanced?
        while (charge_counter~=0)                                     %Loop until balanced
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %This portion of code serves to search the box
            %perimeter,这段代码用来搜索
            %
...
if c1==c2                                   %Cater for the infinite gradient斜率为无穷大
    if r2>r1
        for i=1:radius
            r_fill=r1 + i;
...
%I'm sure there is a better way of doing it...可能有更好的方法改进)
if theta<0 && c2<c1
